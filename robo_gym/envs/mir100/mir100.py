...
class TrajectoryNavigationMir100(Mir100Env):
    ...
    def __init__(self, rs_address=None, **kwargs):
        ...
        self.action_space = spaces.Box(low=np.full((1), 50), high=np.full((1), 1000), dtype=np.float32)
    ...
    def reset(self, start_pose = None, target_pose = None):
        ...
        # DEBUG:
        start_pose  = [0.0, 0.0, 1.0]
        target_pose = [5.0, 8.0, 0.0]
    ...
    def step(self, action):
        ...
        trajectory_time = self.io_sfl(self.start_pose, self.target_pose, action)
